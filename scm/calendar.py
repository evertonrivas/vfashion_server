...
class CalendarFLIMV(Resource):
    @ns_calendar.response(HTTPStatus.OK.value,"Obtem a listagem de flimvs")
    @ns_calendar.response(HTTPStatus.BAD_REQUEST.value,"Falha ao listar registros!")
    @ns_calendar.param("id","Id para busca de um registro","query",type=int,required=False)
    @auth.login_required
    def get(self):
    ...
    @ns_calendar.response(HTTPStatus.OK.value,"Salva ou atualiza um flimv")
    @ns_calendar.response(HTTPStatus.BAD_REQUEST.value,"Falha ao salvar ou atualizar registro!")
    @auth.login_required
    def post(self):
        req = request.get_json()
        try:
...
ns_calendar.add_resource(CalendarFLIMV,'/flimv')