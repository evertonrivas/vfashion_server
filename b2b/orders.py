...
class OrdersList(Resource):
    ...
    @ns_order.param("page","Número da página de registros","query",type=int,required=True,default=1)
    @ns_order.param("pageSize","Número de registros por página","query",type=int,required=True,default=25)
    @ns_order.param("query","Texto a ser buscado","query")
    def get(self):
        ...
        if search=="":
            ...
            #pensar nos filtros para colocar aqui
            rquery = B2bOrders.query.filter(B2bOrders.trash==False).paginate(page=pag_num,per_page=pag_size)
        return {
            "pagination":{
    ...
    def post(self)->int:
        try:
            ...
            order.id_payment_condition = req.id_payment_condition
