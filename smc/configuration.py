...
ns_configuration = Namespace("configuration",description="Operações para manipular as configuracoes do sistema")

#API Models
config_pag_model = ns_configuration.model(
    "Pagination",{
...
config_model = ns_configuration.model(
    "City",{
        "id": fields.Integer,
        "name": fields.String
    }
...
config_return = ns_configuration.model(
    "CityReturn",{
...
# class CategoryList(Resource):
    # @ns_configuration.response(HTTPStatus.OK,"Obtem a listagem de configuracoes",config_return)
    # @ns_configuration.response(HTTPStatus.BAD_REQUEST,"Falha ao listar registros!")
    # @ns_configuration.param("page","Número da página de registros","query",type=int,required=True,default=1)
    # @ns_configuration.param("pageSize","Número de registros por página","query",type=int,required=True,default=25)
    # @ns_configuration.param("query","Texto para busca","query")
    # @auth.login_required
    # def get(self):
    ...
    # @ns_configuration.response(HTTPStatus.OK,"Cria uma nova cidade")
    # @ns_configuration.response(HTTPStatus.BAD_REQUEST,"Falha ao criar novo registro!")
    # @ns_configuration.doc(body=config_model)
    # @auth.login_required
    # def post(self):
    #     try:
...
@ns_configuration.route("/<string:id>")
class CategoryApi(Resource):
    @ns_configuration.response(HTTPStatus.OK,"Obtem um registro de uma nova cidade",config_model)
    @ns_configuration.response(HTTPStatus.BAD_REQUEST,"Registro não encontrado!")
    # @auth.login_required
    def get(self,id:str):
        try:
    ...
    @ns_configuration.response(HTTPStatus.OK,"Atualiza os dados da configuração de um cliente")
    @ns_configuration.response(HTTPStatus.BAD_REQUEST,"Registro não encontrado!")
    @auth.login_required
    ...
    # @ns_configuration.response(HTTPStatus.OK,"Exclui os dados de uma cidade")
    # @ns_configuration.response(HTTPStatus.BAD_REQUEST,"Registro não encontrado!")
    # @auth.login_required
